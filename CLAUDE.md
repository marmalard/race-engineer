...
### Test Suite
- 1151 tests passing on master with local fixtures (`uv run pytest -q` or `.venv/Scripts/python.exe -m pytest -q`); skip count varies with local gitignored fixtures (race-capture integration tests need Oulton; some lap tests need specific telemetry files). PTT engineer merge added 61 engineer/voice tests.
